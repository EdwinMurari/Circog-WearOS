...
import org.hcilab.circog_watch.CircogPrefs;

import org.hcilab.circog_watch.R;
import org.hcilab.circog_watch.Util;
...
class LogThread implements Runnable {
    ...
    private void writeToFile(LogMessage message) {
        try {
            ...
            if (CircogPrefs.DEBUG_MODE) {
                Log.e(TAG, e.getMessage() + "\ncould not write message to file", e);
            }
        }
    ...
    public void updateTimeStamps(Map<String, String> map) {
        ...
        final String nowStr = Util.getTimeStringMySql(now);
        final long uptimeSec = Util.getUpTimeSec();

        map.put(Keys.UID, deviceUid);
        map.put(Keys.KEY_T, String.valueOf(now));
        map.put(KEY_UPTIME_SINCE_START_S, String.valueOf(uptimeSec));
        map.put(KEY_DATE, nowStr);
    }
    ...
    public void updateTimeStamps(JSONObject json) {
        ...
        try {
            ...
            json.put(KEY_UPTIME_SINCE_START_S, String.valueOf(uptimeSec));
            json.put(KEY_DATE, nowStr);
        } catch (JSONException e) {
            if (CircogPrefs.DEBUG_MODE) {
