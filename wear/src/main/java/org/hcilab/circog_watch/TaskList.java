...
public class TaskList {
    ...
    public static void incrementDailyTaskCount(Context context) {
        ...
        int taskCountToBeSent = 0;
        if(Util.isSameDay(lastTaskCompleted, nowDate)) { //reset task count if last task was completed on any other day than today
            taskCountToBeSent = ++dailyTaskCount;
            Util.putInt(context, CircogPrefs.DAILY_TASK_COUNT, taskCountToBeSent);
        } else {
            dailyTaskCount = 1;
            taskCountToBeSent = dailyTaskCount;
        ...
        // Update task details data on handheld
        sendTaskCountDetailsToHandheld(context, taskCountToBeSent, now);
    ...
    private static void sendTaskCountDetailsToHandheld(Context context, int taskCount, long now) {
        PutDataMapRequest dataMap = PutDataMapRequest.create(CircogPrefs.TASK_DETAILS_PATH);

        dataMap.getDataMap().putInt(CircogPrefs.TASK_COUNT_KEY, taskCount);
        dataMap.getDataMap().putLong(CircogPrefs.LAST_TASK_TIME_KEY, now);
        ...
        dataItemTask.addOnSuccessListener(
                new OnSuccessListener<DataItem>() {
                    ...
                    public void onSuccess(DataItem dataItem) {
                        if (CircogPrefs.DEBUG_MODE) {
                            Log.i(TAG, "Sending task details message was successful: " + dataItem);
                        }
                    }
                });
    }
